...
$firstTotal = $cart->total() + $costoEnvio;
$saldoADescontar = $cart->total() + $costoEnvio;

if ($_SESSION['current_customer_puntos'] >= 100) {
    $descuento = 10;
    $conPuntos = true;
    $firstTotal = $firstTotal - 10;
    $saldoADescontar = $saldoADescontar - 10;
    $observaciones = $observaciones . " - " . "(PEDIDO PAGADO PARCIALMENTE CON PUNTOS)";
}
...
if ($conPuntos) {
    $objCliente->reducirPuntosCliente($_SESSION['current_customer_idCliente'], 100);
}

$objCliente->reducirSaldoCliente($_SESSION['current_customer_idCliente'], $saldoADescontar);
$objConsumo->addNewConsumo($_SESSION['current_customer_idCliente'], $saldoADescontar, 'WEB');
...
if ($_SESSION['envio'] == 'recojo' || $_SESSION['solo_gift_cards'] == 'true') {
    $pedidoInsertado = $objPedido->addPedido($customerID,
        $direccion, $telefono, $observaciones, 0, $puntosGanados,
        '', '', $horaPedido, '', $totalPipe, 'false', '-', $_SESSION['current_customer_tipoDocumento'], $razonSocial, $direccionFiscal, $ruc, $latitud, $longitud, 0, $idTienda);
} else {
    $pedidoInsertado = $objPedido->addPedido($customerID,
        $direccion, $telefono, $observaciones, 0, $puntosGanados,
        '', '', $horaPedido, '', $totalPipe, 'true', $distrito, $_SESSION['current_customer_tipoDocumento'], $razonSocial, $direccionFiscal, $ruc, $latitud, $longitud, $costoEnvio, $idTienda);
}
...
foreach ($cartItems as $item) {
    ...
    elseif ($item['giftTipo'] == 'regalo') {
        ...
        if ($countCustomer > 0) {
            ...
            $objCliente->updateClienteSaldo($clienteEXistente['idCliente'], $item['subtotalGiftValor']);
                        ...
                        <table width="480" border="0" cellpadding="0" cellspacing="0" align="center"
                            ...
                            <tr>
                                <td style="font-size: 12px; font-family: \'Open Sans\', sans-serif; color: #000; line-height: 22px; vertical-align: top; text-align:right; ">
                                    <strong>Total (Incl.Envio)-Saldo</strong>
                                </td>
                                <td style="font-size: 12px; font-family: \'Open Sans\', sans-serif; color: #000; line-height: 22px; vertical-align: top; text-align:right; ">
